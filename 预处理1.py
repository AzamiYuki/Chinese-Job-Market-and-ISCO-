...
# 输出文件路径
OUTPUT_CSV_FILE = '/Users/wyh/Desktop/统计计算/newjob1_isco_4digit_code_only.csv'

# CSV中需要读取的列名
COLUMNS_TO_READ = ['岗位', '岗位描述', '岗位职能']
# 新增的列名
NEW_COLUMN_CODE = 'ISCO_4_Digit_Code'

# API调用间的等待时间（秒），避免频率限制
API_CALL_DELAY = 0.6

# 处理的最大行数 (设置为None则处理所有行, 设置为具体数字如1000则只处理前1000行)
MAX_ROWS_TO_PROCESS = 10000 # 设置为 50000 或 None 处理全部
...
def query_deepseek_for_isco_code(job_title, job_description, job_function):
    """
    调用DeepSeek API获取ISCO四级分类代码。
    """
    prompt_content = f"""
...
def process_csv_classification():
    """
    读取CSV文件，进行ISCO四级分类（仅编码），并将结果写入新的CSV文件。
    """
    processed_rows_count = 0
    print(f"准备处理文件: {INPUT_CSV_FILE}")
    ...
    try:
        with open(INPUT_CSV_FILE, mode='r', encoding='utf-8-sig') as infile, \
            ...
            for i, row in enumerate(reader):
                ...
                # print(f"正在处理第 {current_row_number} 行...") # 可以取消注释以获得更详细的逐行日志
                ...
                if job_title or job_description or job_function:
                    raw_api_response = query_deepseek_for_isco_code(job_title, job_description, job_function)
                    api_isco_code = validate_api_response_code(raw_api_response)
                    if processed_rows_count % 10 == 0 : # 每10行打印一次API的返回，方便监控
                         print(f"  第 {current_row_number} 行 - API返回编码: {api_isco_code}")
                else:
                ...
                # 将新获取的数据添加到行中
                row_to_write = row.copy() # 创建副本以修改
                row_to_write[NEW_COLUMN_CODE] = api_isco_code
                
                writer.writerow(row_to_write)
                processed_rows_count += 1

                if processed_rows_count % 100 == 0: # 每100行提示一次进度
                    print(f"  已处理 {processed_rows_count} 行。正在写入文件...")
                    outfile.flush() # 定期刷新缓冲区到磁盘
...
if __name__ == "__main__":
    if not API_KEY or API_KEY == "sk-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx": 
        print("错误：请在脚本顶部将 'API_KEY' 变量替换为您的有效DeepSeek API密钥。")
